...
def main() -> None:
    ...
    md: list[str] = [
        ...
        "",
        f"This analysis aggregates **{len(run_dirs)} independent full benchmark runs**.",
        "",
        "Values below are reported as **mean +/- sample standard deviation across runs**. This is separate from the within-run latency standard deviation already preserved in each raw JSON result.",
        "",
    ...
    md += [
        ...
        "## Interpretation",
        "",
        "Run-to-run variance is expected in public managed services because network conditions, provider scheduling, throttling and shared infrastructure can change between executions. Reporting multiple independent runs therefore makes the comparison more defensible than relying on one production run alone.",
        "",
